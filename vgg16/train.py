...
def train(args):
    ...
    model = VGG16D(len(train_data.categories)).to(device)
    criterion = CrossEntropyLoss()
    optimizer = SGD(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=5.e-4)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        plot_confusion_matrix(writer, confusion_matrix(list_labels, list_predictions), class_names=val_data.categories, epoch=epoch)
